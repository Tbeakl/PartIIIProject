...
number_of_bits::Int64 = parse(Int64, ARGS[1])
...
keyset_fid = h5open("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/evaluation/key_sets/100_random_keys_nonces_counter.hdf5", "r")
for i in 1:100
...
for i in 1:100
    ...
    dimensions::Int64 = 8
    signal_to_noise_ratio::Float64 = 1.5
    key::Vector{UInt32} = all_keys[i] #generate_random_key()
    nonce::Vector{UInt32} = all_nonces[i] #generate_random_nonce()
    counter::UInt32 = all_counters[i] #generate_random_counter()
    ...
    initial_number_of_iterations = 3 # 200
    ...
    for i in 1:initial_number_of_iterations
        ...
        push!(tot_entropy_over_time, total_entropy_of_graph(variables))
        println(tot_entropy_over_time[end])
        if tot_entropy_over_time[end] < 1 || abs(tot_entropy_over_time[end] - tot_entropy_over_time[end-1]) <= 0.01
            break
        end
    end
    ...
    push!(successes, read_off_key == key)
end

successes_fid = h5open(string("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/evaluation/tree_adds/", number_of_bits, "_successes.hdf5"), "w")
successes_fid["successes"] = successes
close(successes_fid)
