...
function add_qr_leakage(trace::Vector{Float32},
    ...
    add_distribution_from_leakage_to_point(trace, variables, factors, bits_per_cluster, location_execution_counts, a, run_number, trace_value_to_graph)
    # Need to do the part with shifting
    position_in_trace += 3
    if 16 % bits_per_cluster != 0
        # This means that the result of the rotation is directly going to be the output after the rotation
    ...
    add_distribution_from_leakage_to_point(trace, variables, factors, bits_per_cluster, location_execution_counts, d, run_number, trace_value_to_graph)

    add_distribution_from_leakage_to_point(trace, variables, factors, bits_per_cluster, location_execution_counts, c, run_number, trace_value_to_graph)
    position_in_trace += 3
    if 12 % bits_per_cluster != 0
        location_execution_counts[b] += 1
    end
...
function add_leakage_trace_to_factor_graph(trace::Vector{Float32},
    ...
    location_execution_counts = ones(Int64, 16)
    for i in 1:10
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 1, 5, 9, 13, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 2, 6, 10, 14, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 3, 7, 11, 15, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 4, 8, 12, 16, location_execution_counts, run_number, trace_value_to_graph)

        add_qr_leakage(trace, variables, factors, bits_per_cluster, 1, 6, 11, 16, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 2, 7, 12, 13, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 3, 8, 9, 14, location_execution_counts, run_number, trace_value_to_graph)
        add_qr_leakage(trace, variables, factors, bits_per_cluster, 4, 5, 10, 15, location_execution_counts, run_number, trace_value_to_graph)
    end
    # Need to have another part for setting the values to be exactly what they were in the output of the function because
    # we assume that we have access to those actual values (or at least a better leakage) compared to just their leakage values
end
