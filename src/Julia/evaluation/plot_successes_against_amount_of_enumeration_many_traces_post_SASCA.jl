...
for i in 1:2:length(base_paths_to_counts)
    cur_colors = get_color_palette(:auto, plot_color(:white))
    plot!(p, final_ranks[i], proportion, label=labels[i], linewidth=1.5, c=cur_colors[(i ÷ 2) + 1], linestyle=:solid)
    plot!(p, final_ranks[i + 1], proportion, label=labels[i + 1], linewidth=1.5, c=cur_colors[(i ÷ 2) + 1], linestyle=:dash)
end
p
savefig(p, "./plots/evaluation/real_attack_multi_trace_post_SASCA.pdf")