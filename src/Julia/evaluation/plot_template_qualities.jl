...
p = heatmap(heatmap_of_counts, #title="First-order success rate",
 ylabel="ChaCha state", xlabel="Operation number", dpi=300,
...
p = heatmap(heatmap_of_counts, #title="Logarithmic guessing entropy",
 ylabel="ChaCha state", xlabel="Operation number", clim=(0,7), dpi=300,
yticks=([0.5:(32 ÷ number_of_bits):((512 ÷ number_of_bits) + 1);], 0:16),
guidefontsize=guidefontsize)
...
heatmap_of_counts = map_to_values.(mapped_matrix, Ref(log2.(guessing_entropies)), Ref(32 ÷ number_of_bits))
p = heatmap(heatmap_of_counts, #title="Logarithmic guessing entropy",
 ylabel="ChaCha state", xlabel="Operation number", clim=(0,15), dpi=300,
yticks=([0.5:(32 ÷ number_of_bits):((512 ÷ number_of_bits) + 1);], 0:16),
guidefontsize=guidefontsize)
