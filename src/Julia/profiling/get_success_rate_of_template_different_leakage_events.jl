...
function get_number_of_guesses(value::Int64, prob_table::AbstractVector)
    permutation_of_results = sortperm(prob_table; rev=true)
    return findfirst(x -> x == value, (0:(length(permutation_of_results)-1))[permutation_of_results])
end
...
for intermediate_value_index in 1:number_of_itermediate_values
    for template_number in 1:number_of_templates_per_intermediate_value
        ...
        all_prob_tables = zeros(length(intermediate_value_vector), 1 << number_of_bits_per_template)
        template_attack_location = 1
        template_path = string("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/attack_profiling/second_trace_set/initial_templates_LR_leakage_event/sparse_50_detailed_50/",
            intermediate_value_index, "_", template_number, "_", template_attack_location, "_template.hdf5")
        while ispath(template_path)
            println(template_attack_location)
            fid = h5open(template_path, "r")
            cov_matrix = read(fid["covariance_matrix"])
            number_of_dimensions = 8 #min(8, size(cov_matrix)[1])
            ...
            close(fid)
            noise_distribution = noise_distribution_fixed_standard_dev(1.0, number_of_dimensions)
            original_traces = downsampled_matrix[:, sample_bitmask]
            projected_vectors = original_traces * template_projection
            # Need to go through all the traces and together their distributions to see what happens
            for i in eachindex(intermediate_value_vector)
                all_prob_tables[i, :] += get_prob_dist_of_vector(mean_vectors, noise_distribution, projected_vectors[i, :])
            end
            template_attack_location += 1
            template_path = string("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/attack_profiling/second_trace_set/initial_templates_LR_leakage_event/sparse_50_detailed_50/",
                intermediate_value_index, "_", template_number, "_", template_attack_location, "_template.hdf5")
        end
        success_rates[4*(intermediate_value_index-1)+template_number] = mean(get_number_of_guesses.(intermediate_value_vector, eachrow(all_prob_tables)))#get_guessing_entropy(mean_vectors, noise_distribution, intermediate_value_vector, projected_vectors)
    end
end
...
open("average_guessing_entropy_constant_attack_leakage_events.csv", "w") do file
    # First do the key
    write(file, "Key\n")
    for i in 5:12
