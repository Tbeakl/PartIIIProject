...
interesting_samples = original_matrix

success_rates_mean = zeros(Int64, 3, 6)
success_rates_nicv = zeros(Int64, 3, 6)
...
for intermediate_index in base_intermediate_value_index:(base_intermediate_value_index + 3)
    ...
    for (dilation_number, number_of_samples_dilation_averaged_over) in enumerate([100, 250, 500])
        ...
        for (i, number_of_samples_to_mean_over) in enumerate([1,2,5,10, 20, 50, 100, 250, 500]) #1, 2, 5, 
            downsampled_matrix = reduce(vcat, transpose.(downsample_to_mean.(eachrow(interesting_samples), number_of_samples_to_mean_over)))
            all_correct_counts = 0
            template_path = string("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/attack_profiling/initial_templates_all_cycles_", intermediate_index, "/", number_of_samples_to_mean_over, "_points_template.hdf5")
            ...
            all_correct_counts = 0
            template_path = string("D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/attack_profiling/initial_templates_all_cycles_", intermediate_index, "/", number_of_samples_to_mean_over, "_weighted_points_template.hdf5")
            intermediate_value_vector = all_intermediate_values[:, intermediate_index-base_intermediate_value_index+1]
            fid = h5open(template_path, "r")
            template_projection = read(fid["projection"])
            mean_vectors = read(fid["class_means"])
            ...
            nicv = read(fid["NICV"])
            close(fid)
            downsampled_matrix = reduce(vcat, transpose.(downsample_to_mean_weighted_by_NICV.(eachrow(interesting_samples), Ref(nicv), number_of_samples_to_mean_over)))
