...
number_of_clock_cycles = 6498 #15997 #(759500 ÷ 50)
...
for intermediate_value_index in 1:number_of_intermediate_values
    fid = h5open(string(path_to_data * "attack_profiling/32_volatile/correlations/", intermediate_value_index, ".hdf5"), "r")
...
for intermediate_value_index in 1:number_of_intermediate_values
    bitmask = sum(all_mean_NICV[:, (templates_per_intermediate_value * (intermediate_value_index - 1)) .+ (1:templates_per_intermediate_value) ], dims=2) .>= 0.04
    for cluster_num in 1:templates_per_intermediate_value
        ...
        all_bitmasks[:, (templates_per_intermediate_value*(intermediate_value_index-1))+cluster_num] = bitmask
    end
end
...
heatmap_of_counts = map_to_values.(mapped_matrix, Ref(sum(all_bitmasks, dims=1)[1, :]), Ref(32 ÷ number_of_bits))
...
fid = h5open(path_to_data * "attack_profiling/32_volatile/clock_cycles_bitmasks.hdf5", "w")
for intermediate_value_index in 1:number_of_intermediate_values
    for cluster_num in 1:templates_per_intermediate_value
        println(intermediate_value_index, " ", cluster_num)
