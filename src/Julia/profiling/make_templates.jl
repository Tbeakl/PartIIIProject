...
path_to_templates = "D:/Year_4_Part_3/Dissertation/SourceCode/PartIIIProject/data/attack_profiling/second_trace_set/initial_templates_HW/"
    ...
    for sparse_level_index in detailed_level_index:length(different_detail_levels)
        ...
        for intermediate_value_index in 1:number_of_intermediate_values
            for template_num in 1:number_of_templates_per_intermediate_value
                if !ispath(string(path_to_templates, "sparse_", different_detail_levels[sparse_level_index], "_detailed_", different_detail_levels[detailed_level_index], "/", intermediate_value_index, "_", template_num, "_template.hdf5"))
                    println(intermediate_value_index, " ", template_num)
                    bitmask_fid = h5open(bitmask_path, "r")
                    undilated_cycle_bitmask = read(bitmask_fid[string("bitmask_", intermediate_value_index, "_", template_num)])
                    ...
                    template_intermediate_value_vector = Base.count_ones.((intermediate_value_vector .>> (number_of_bits_per_template * (template_num - 1))) .& ((1 << number_of_bits_per_template) - 1))
                    ...
                    fid = h5open(string(path_to_templates, "sparse_", different_detail_levels[sparse_level_index], "_detailed_", different_detail_levels[detailed_level_index], "/", intermediate_value_index, "_", template_num, "_template.hdf5"), "w")
